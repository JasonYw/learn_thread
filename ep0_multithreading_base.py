...
#cpu有很多进程，进程包含线程，进程通过加锁控制进程
#后台线程 前台线程 父线程 子线程
#只要父线程结束，不管子线程什么状态，都要一起走
#前台线程，给父线程做阻塞，等前台线程做完在一起走
#队列，队列相当于内存的堆，先进先出，栈是后进先出
#put往队列里放参数，get取参数
#创建线程，通过队列的方式来保存数据
#创建多个线程，重写父线程。
#多进程用于密集的计算
...
#一个核心只能执行一个任务
#多个cpu核心同时可以执行多个任务
#一个cpu同时只能执行一个进程，当一个进程执行的时候，其他进程是非运行状态
#进程里包含的执行单元叫线程
#一个进程包含多个线程

#一个进程的内存空间是共享的，每个进程里的线程都可以使用共享空间
#一个线程在使用共享空间的时候，其他进程必须等到他结束。
#通过锁实现的，作用防止多个线程使用这块内存空间
#先使用的线程会将空间上锁，其他线程在门口等待，打开锁再进去
#进程：表示程序一次执行
#线程：表示cpu运算的基本调度单位

#GIL全局解释锁：通行证python里的执行通行证，只有一个，拿到通行证的线程就可以进入cpu执行，否则就无法执行
#python的多线程：适用于大量密集的io处理
#python多进程：大量的密集并行计算
#爬虫适用于多线程
#import Queue
#创建对象
#队列一个 Queue.get()   Queue. put()   Queue.empty() 判断队列是否为空 Queue.full() 判断队列是否满了
#Queue.qsize() 判断队列大小
#  #scrapy
